"""Admin panel — /admin command + saare management flows."""
import asyncio
import logging
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if a == "addstock_pick":
        ...
        return await _edit(q, "➕ Kis product mein stock add karna hai?",
                           InlineKeyboardMarkup(kb))
    if a == "addstock":
        st.set_state(uid, st.S_ADDSTOCK, pid=int(parts[2]))
        return await _edit(
            q,
            "➕ <b>Add Stock</b>\n\nHar line mein 1 item bhejein (account / link / code):\n\n"
            "<code>user1:pass1\nuser2:pass2</code>",
            back_kb("adm:home"))
    if a == "rename":
    ...
    if a == "sale":
        ...
        return await _edit(
            ...
            "Sale set hote hi tamam users ko notification chali jayegi.\n"
    ...
    if a == "deposits":
        ...
        kb = []
        for d in deps[:12]:
            text += (f"#{d['id']} — <b>${d['amount']:g}</b> via {d['method']}\n"
                     f"👤 {d['first_name'] or ''} @{d['username'] or '-'} "
                     f"(<code>{d['user_id']}</code>)\n"
                     f"🧾 {d['tx_info'][:60] or 'cryptobot invoice'}\n\n")
    ...
    if a == "dep_ok":
        ...
        db.set_deposit_status(did, "paid")
        db.add_balance(d["user_id"], d["amount"])
        u2 = db.get_user(d["user_id"])
        # referral commission
        if u2 and u2.get("referrer_id"):
            try:
                ...
                pct = 0
            bonus = round(d["amount"] * pct / 100, 6)
            if bonus > 0:
                ...
                try:
                    await context.bot.send_message(
                        ...
                        f"Aapke referral ne ${d['amount']:g} deposit kiya.\n"
                        f"🎁 Aapko <b>${bonus:g}</b> ({pct:g}%) mile!",
                        parse_mode="HTML")
                except Exception:
                    pass
        try:
            await context.bot.send_message(
                ...
                f"💳 New balance: <b>${u2['balance']:g}</b>",
                parse_mode="HTML")
        except Exception:
            pass
        return await _edit(q, f"✅ Deposit #{did} approved — ${d['amount']:g} credited.",
                           back_kb("adm:deposits"))
    if a == "dep_no":
        did = int(parts[2])
        d = db.get_deposit(did)
        ...
        try:
            await context.bot.send_message(
                d["user_id"],
                f"❌ Aapki deposit #{did} reject ho gayi. Masla ho to support se rabta karein.")
        except Exception:
            pass
    ...
    if a == "broadcast":
        ...
        return await _edit(
            q,
            "📣 <b>Broadcast</b>\n\nJo message ab bhejein ge (text / formatting) wo "
...
async def admin_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == st.S_ADDPROD_NAME:
        ...
        await update.message.reply_text(
            "Step 2/3: <b>Price</b> bhejein (USDT, masalan <code>4.5</code>):",
            parse_mode="HTML")
        ...
        await update.message.reply_text(
            "Step 3/3: <b>Stock items</b> bhejein — har line mein 1 item.\n"
            "Stock baad mein add karne ke liye <code>skip</code> likhein.",
            parse_mode="HTML")
        ...
        st.clear(uid)
        await update.message.reply_text(
            f"🔥 Flash sale set! {sent} users ko notification gayi.",
            reply_markup=back_kb(f"adm:prod:{pid}"))
        ...
        if not u2:
            return await update.message.reply_text(
                "❌ Format: <code>user_id amount</code> (user nahi mila?)",
                parse_mode="HTML")
        ...
        await update.message.reply_text(
            ...
            f"new balance: ${db.get_user(u2['user_id'])['balance']:g}",
            parse_mode="HTML")
        ...
        await update.message.reply_text(
            ...
            f"{target['first_name']} (<code>{target['user_id']}</code>)",
            parse_mode="HTML")
    elif state == st.S_LOOKUP:
        target = _parse_target(db, text)
        if not target:
            return await update.message.reply_text("❌ User nahi mila.")
        orders, spent = db.order_stats(target["user_id"])
        st.clear(uid)
        await update.message.reply_text(
            f"👤 <b>User Profile</b>\n\n"
